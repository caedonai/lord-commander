...
lord-commander init --quick --type=full-stack --dashboard-path ./admin --api-path ./backend
# Creates: ./cli, ./backend, ./admin

# Mixed approach - output folder with custom API path
lord-commander init --quick --type=cli-api --output my-project --api-path ../shared-api
# Creates: ./my-project/cli, ../shared-api
```
...
```
. (current directory)
├── package.json         # Your existing package.json updated
├── cli/                 # CLI application
├── api/                 # Backend API
└── dashboard-ui/        # Frontend dashboard
```
...
```
. (current directory)
├── cli/                 # CLI application (default location)
├── server/              # Backend API (custom path)
└── admin/               # Frontend dashboard (custom path)
```
...
5. **Project Structure** (for multi-component setups)
   ...
   ❯ Create in new project folder (recommended)
...
```
my-project/              # ← Project folder created
├── package.json         # Workspace configuration
├── README.md
├── cli/
│   ├── src/
...
│   │   └── services/
│   └── package.json
└── dashboard-ui/        # Admin web interface
    ├── src/
    ├── public/
    └── package.json
```
...
├── package.json         # Updated workspace config
├── cli/                 # CLI application
├── server/              # API at custom path
└── admin/               # Dashboard at custom path
```
...
# ⚠️  Use with caution: Current directory (may clutter workspace)
lord-commander init --quick --type=full-stack

# 🔧 Advanced: Custom paths (for specific requirements)
lord-commander init --quick --type=full-stack --dashboard-path ./admin --api-path ./backend
```
...
📁 Project created at: /path/to/my-awesome-project
📂 Project structure:
...
📚 Next steps:
• Navigate to your project: cd my-awesome-project
...
🐛 Issues: https://github.com/caedonai/lord-commander/issues
```
...
📁 Project created at: /current/directory
📂 Project structure:
...
• See individual README files for component-specific instructions
```
...
<Tabs>
  ...
  <Tab title="Advanced Patterns">
    ```bash
    # Microservices pattern - separate API repository
    lord-commander init --quick --type=cli-api --api-path ../shared-api
    ...
    lord-commander init --quick --type=full-stack --dashboard-path ./admin --api-path ../existing-api
    
    # Enterprise workspace structure
    lord-commander init --quick --type=full-stack --output apps/my-tool --dashboard-path ./apps/admin --api-path ./services/api
    ```
  </Tab>
</Tabs>
