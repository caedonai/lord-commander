...
const createFile = async (filePath: string, content: string) => {
  ...
  if (!isPathSafe(resolvedPath, workingDir)) {
    throw new Error('Invalid file path: cannot access files outside project directory');
  }
...
const makeSecureRequest = async (urlString: string, options: RequestOptions = {}) => {
...
class TokenManager {
  ...
  static async getValidToken(): Promise<string> {
    ...
    if (!tokenJson) {
      throw new Error('No authentication token found. Please run: lord-commander auth login');
    }
