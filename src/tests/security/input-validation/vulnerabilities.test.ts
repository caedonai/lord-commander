...
describe('Input Validation Security Vulnerabilities', () => {
  ...
  describe('5. Command Injection via Package Manager Validation', () => {
    it('should reject package managers with executable extensions', () => {
      const maliciousPackageManagers = ['npm.exe', 'yarn.cmd', 'pnpm.bat', 'rush.ps1', 'nx.sh'];
    ...
    it('should reject package managers with path separators', () => {
      ...
      pathInjectionAttempts.forEach((pm) => {
        ...
        const hasSecurityViolation = result.violations.some(
          (v) => v.type === 'path-traversal' || v.type === 'suspicious-pattern'
        );
  ...
  describe('6. TOCTOU Race Condition', () => {
    it('should validate path atomically', () => {
      ...
      const results = Array.from({ length: 50 }, () => validateInput(testPath, 'file-path'));
