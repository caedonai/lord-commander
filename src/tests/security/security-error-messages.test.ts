...
describe('ERROR_MESSAGES Security Constants', () => {
  describe('Existing error messages', () => {
    it('should generate INVALID_COMMAND_PATH error message', () => {
      ...
      expect(result).toContain('Command paths must be within the current working directory for security');
    });
  ...
  describe('New security-focused error messages', () => {
    ...
    it('should generate UNSAFE_TEMPLATE_SOURCE error message', () => {
      ...
      expect(result).toContain('Template source not whitelisted: http://malicious-site.com/template.zip');
      expect(result).toContain('Only verified sources allowed');
      expect(result).toContain('Please use official templates or add this source to your allowlist');
    });
    ...
    it('should generate CONFIGURATION_TAMPERING error message', () => {
      ...
      expect(result).toContain('Configuration tampering detected in package.json: malicious postinstall script');
  ...
  describe('Error message type safety', () => {
    it('should ensure all error message functions return strings', () => {
      ...
      expect(typeof ERROR_MESSAGES.COMMAND_NAME_CONFLICT('test', 'path1', 'src1', 'path2', 'src2')).toBe('string');
