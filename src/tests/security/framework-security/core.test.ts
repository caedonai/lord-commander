...
describe('Framework Security Detection', () => {
  ...
  describe('Dependency Security Analysis', () => {
    ...
    it('should handle malformed package.json', async () => {
      ...
      expect(result).toBeDefined();
      expect(result?.dependencies.security.untrustedSources).toContain(
  ...
  describe('Security Utility Functions', () => {
    it('should provide security recommendations', () => {
      ...
      expect(recommendations).toContain(
        'Framework failed security validation - consider manual review'
      );
  ...
  describe('Security Pattern Constants', () => {
    ...
    it('should detect suspicious dependency patterns', () => {
      ...
      suspiciousNames.forEach((name) => {
        const isSuspicious = SUSPICIOUS_DEPENDENCY_PATTERNS.some((pattern: RegExp) =>
          pattern.test(name)
        );
    ...
    it('should detect dangerous script patterns', () => {
      ...
      dangerousScripts.forEach((script) => {
        const isDangerous = DANGEROUS_SCRIPT_PATTERNS.some((pattern: RegExp) =>
          pattern.test(script)
        );
  ...
  describe('Next-forge Style Projects', () => {
    it('should handle trusted + unknown dependencies gracefully', async () => {
      ...
      if (framework) {
        ...
        expect(recommendations).toContain(
          'Audit unknown dependencies for security vulnerabilities'
        );
      }
    });
  ...
  describe('Dependency Pattern Validation', () => {
    it('should correctly identify all suspicious dependency patterns', () => {
      const suspiciousNames = [
        'evil-package',
        'malicious-lib',
        'hack-tool', // Prefix patterns
        'backdoor-util',
        'trojan-horse',
        'virus-scanner', // Content patterns
        'test-lib',
        'demo-app',
        'example-util', // Typosquatting patterns
        '.hidden-pkg',
        '_internal-lib', // Hidden patterns
        '123abc456',
        'a1b2c3', // Version-like patterns
        'a',
        'x',
        'zz', // Very short names
      ];
      ...
      suspiciousNames.forEach((name) => {
        const isSuspicious = SUSPICIOUS_DEPENDENCY_PATTERNS.some((pattern) => pattern.test(name));
        expect(isSuspicious).toBe(true);
        if (!isSuspicious) {
    ...
    it('should not flag legitimate dependency names', () => {
      const legitimateNames = [
        ...
        '@types/node',
        '@babel/core',
        '@next/bundle-analyzer', // Scoped packages
        'eslint-config-next',
        'postcss-preset-env', // Hyphenated packages
        'create-react-app',
        'webpack-dev-server', // Multi-word packages
      ];
