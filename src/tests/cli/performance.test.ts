...
describe('CLI Performance Tests', () => {
  ...
  describe('Command Performance', () => {
    it.each(performanceTests)('should execute $name within performance limits', async ({ command, iterations, maxTime }) => {
      const times: number[] = [];
      ...
      console.log(`${command.join(' ')} - Avg: ${Math.round(avgTime)}ms, Min: ${Math.round(minTime)}ms, Max: ${Math.round(maxTimeActual)}ms`);
