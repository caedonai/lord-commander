 ...
 */
async function createMockWorkspace(type: 'nx' | 'lerna' | 'rush' | 'pnpm' | 'yarn' | 'npm' | 'multi'): Promise<MockWorkspace> {
  const workspacePath = path.join(os.tmpdir(), `workspace-test-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);
...
async function createNxWorkspace(workspacePath: string): Promise<void> {
  ...
  await writeFile(path.join(workspacePath, 'nx.json'), JSON.stringify({
    version: 3,
    projects: {
  ...
  await writeFile(path.join(workspacePath, 'apps', 'app-one', 'package.json'), JSON.stringify({
    name: 'app-one',
    version: '1.0.0',
    private: true,
    dependencies: {
  ...
  await writeFile(path.join(workspacePath, 'libs', 'shared', 'package.json'), JSON.stringify({
    name: 'lib-shared',
    version: '1.0.0',
    private: true,
    scripts: {
...
async function createLernaWorkspace(workspacePath: string): Promise<void> {
  ...
  // lerna.json
  await writeFile(path.join(workspacePath, 'lerna.json'), JSON.stringify({
    version: 'independent',
    packages: ['packages/*'],
    npmClient: 'npm',
    command: {
  ...
  await writeFile(path.join(workspacePath, 'packages', 'pkg-a', 'package.json'), JSON.stringify({
    name: 'pkg-a',
    version: '1.2.0',
    dependencies: {
      'pkg-b': '^2.0.0',
  ...
  await writeFile(path.join(workspacePath, 'packages', 'pkg-b', 'package.json'), JSON.stringify({
    name: 'pkg-b',
    version: '2.0.0',
    scripts: {
...
async function createRushWorkspace(workspacePath: string): Promise<void> {
  // rush.json
  await writeFile(path.join(workspacePath, 'rush.json'), JSON.stringify({
    rushVersion: '5.82.0',
    pnpmVersion: '7.15.0',
  ...
  await writeFile(path.join(workspacePath, 'apps', 'service-api', 'package.json'), JSON.stringify({
    name: 'service-api',
    version: '1.0.0',
    dependencies: {
  ...
  await writeFile(path.join(workspacePath, 'libs', 'shared-utils', 'package.json'), JSON.stringify({
    name: 'shared-utils',
    version: '1.0.0',
    scripts: {
...
async function createPnpmWorkspace(workspacePath: string): Promise<void> {
  ...
  // pnpm-workspace.yaml
  await writeFile(path.join(workspacePath, 'pnpm-workspace.yaml'), 
    'packages:\n  - "packages/*"\n  - "apps/*"\n'
  );
  ...
  await writeFile(path.join(workspacePath, 'packages', 'utils', 'package.json'), JSON.stringify({
  ...
  await writeFile(path.join(workspacePath, 'apps', 'web', 'package.json'), JSON.stringify({
    name: '@workspace/web',
    version: '1.0.0',
    dependencies: {
...
async function createYarnWorkspace(workspacePath: string): Promise<void> {
  ...
  await writeFile(path.join(workspacePath, 'packages', 'core', 'package.json'), JSON.stringify({
    name: 'core',
    version: '1.0.0',
    scripts: {
  ...
  await writeFile(path.join(workspacePath, 'packages', 'ui', 'package.json'), JSON.stringify({
    name: 'ui',
    version: '1.0.0',
    dependencies: {
      core: '^1.0.0'
...
async function createNpmWorkspace(workspacePath: string): Promise<void> {
  ...
  await writeFile(path.join(workspacePath, 'packages', 'common', 'package.json'), JSON.stringify({
    name: 'common',
    version: '1.0.0'
  }, null, 2));
}
...
async function createMultiToolWorkspace(workspacePath: string): Promise<void> {
  ...
  await writeFile(path.join(workspacePath, 'turbo.json'), JSON.stringify({
  ...
  await writeFile(path.join(workspacePath, 'packages', 'multi-lib', 'package.json'), JSON.stringify({
    name: 'multi-lib',
    version: '1.0.0',
    scripts: {
  ...
  await writeFile(path.join(workspacePath, 'apps', 'multi-app', 'package.json'), JSON.stringify({
    name: 'multi-app',
    version: '1.0.0',
    dependencies: {
      'multi-lib': '^1.0.0'
...
describe('Workspace Plugin - Comprehensive Tests', () => {
  ...
  describe('Workspace Validation', () => {
    ...
    it('should warn about inconsistent dependency versions', async () => {
      ...
      await writeFile(path.join(mockWorkspace.path, 'apps', 'multi-app2', 'package.json'), JSON.stringify(app2Json, null, 2));
