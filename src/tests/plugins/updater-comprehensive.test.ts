...
import { ensureDir, readFile, writeFile } from '../../core/execution/fs.js';
import {
  applyUpdate,
  createTag,
  createUpdatePlan,
...
async function createTestRepository(): Promise<TestRepository> {
  const testRepoPath = path.join(
    os.tmpdir(),
    `updater-test-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
  );
...
async function createSampleProjectHistory(repoPath: string): Promise<void> {
  ...
  await writeFile(
    path.join(repoPath, 'src', 'config.js'),
  ...
  await execa(
    'git',
    ['commit', '--no-verify', '-m', 'feat: add utility functions and configuration'],
    { cwd: repoPath }
  );
  ...
  await writeFile(
    path.join(repoPath, 'test', 'utils.test.js'),
  ...
  await execa(
    'git',
    ['commit', '--no-verify', '-m', 'fix: add input validation and multiply function'],
    { cwd: repoPath }
  );
  ...
  await execa(
    'git',
    ['commit', '--no-verify', '-m', 'feat: add experimental power function (beta)'],
    { cwd: repoPath }
  );
  ...
  describe('Update Application (Safe Dry-Run)', () => {
    ...
    it('should handle different update strategies in dry-run', async () => {
      ...
      for (const strategyType of strategies) {
        ...
        await expect(applyUpdate(plan, testRepo.path, { dryRun: true })).resolves.not.toThrow();
      }
    });
