...
export class AuditSecurityValidator {
  ...
  static validateAuditEntry(entry: AuditEntry): { isValid: boolean; errors: string[] } {
    ...
    if (entry.userContext) {
      entry.userContext = AuditSecurityValidator.sanitizeContextObject(entry.userContext);
    }
    if (entry.systemContext) {
      entry.systemContext = AuditSecurityValidator.sanitizeContextObject(entry.systemContext);
    }
    if (entry.resourceContext) {
      entry.resourceContext = AuditSecurityValidator.sanitizeContextObject(entry.resourceContext);
    }
        ...
        case 'json': {
          const exportData = {
            ...
            entries: result.entries.map((entry) =>
              AuditSecurityValidator.sanitizeContextObject(entry)
            ), // Sanitize for export
