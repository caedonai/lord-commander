...
export class AdvancedObjectSanitizer {
  ...
  public async sanitizeObject(
    ...
    try {
      ...
      if (processingTime > this.config.maxProcessingTime) {
        warnings.push(`Processing time exceeded limit: ${processingTime}ms > ${this.config.maxProcessingTime}ms`);
      }
      ...
      const report = this.config.generateReport ? 
        await this.generateSanitizationReport(obj, sanitizationResult.sanitized, violations, metrics) : 
        undefined;
  ...
  public async sanitizeBatch(objects: any[]): Promise<ObjectSanitizationResult[]> {
    if (!this.config.enableBatchProcessing || objects.length <= this.config.batchSize) {
      ...
      return Promise.all(objects.map((obj, index) => 
        this.sanitizeObject(obj, `batch[${index}]`)
      ));
    }
    ...
    for (let i = 0; i < objects.length; i += this.config.batchSize) {
      ...
      const batchResults = await Promise.all(
        batch.map((obj, batchIndex) => 
          this.sanitizeObject(obj, `batch[${i + batchIndex}]`)
        )
      );
  ...
  private determineSanitizationStrategy(
    ...
    if (this.config.customStrategies.has(type)) {
      return this.config.customStrategies.get(type)!;
    }
  ...
  private sanitizeString(
    ...
    if (str.length > this.config.maxStringLength) {
      warnings.push(`String truncated at path '${path}': ${str.length} > ${this.config.maxStringLength} chars`);
  ...
  private async sanitizeArray(
    ...
    if (arr.length > this.config.maxArrayLength) {
      warnings.push(`Array truncated at path '${path}': ${arr.length} > ${this.config.maxArrayLength} elements`);
  ...
  private async sanitizePlainObject(
    ...
    if (keys.length > this.config.maxProperties) {
      warnings.push(`${keys.length - this.config.maxProperties} properties truncated at path '${path}': ${keys.length} > ${this.config.maxProperties} properties`);
    }
    ...
    for (const key of limitedKeys) {
      ...
      try {
        ...
        try {
          ...
          warnings.push(`Property sanitization failed at '${path}.${key}': ${sanitizeErrorMessage(propertyError instanceof Error ? propertyError.message : String(propertyError))}`);
        ...
        warnings.push(`Property sanitization failed at '${path}.${key}': ${sanitizeErrorMessage(error instanceof Error ? error.message : String(error))}`);
  ...
  private sanitizeBuffer(buffer: Buffer, path: string, warnings: string[]): any {
    ...
    if (buffer.length > maxBufferSize) {
      warnings.push(`Buffer truncated at path '${path}': ${buffer.length} > ${maxBufferSize} bytes`);
  ...
  private hasDangerousFunctions(obj: any): boolean {
    ...
    for (const value of Object.values(obj)) {
      if (typeof value === 'function') {
        ...
        if (funcStr.includes('eval') || funcStr.includes('Function') || funcStr.includes('setTimeout')) {
          return true;
        }
      }
    }
      ...
      try {
        ...
        for (const key of keys) {
          try {
            ...
            // Property access failed, skip this property but don't assume circular reference
            continue;
          }
        }
      } catch {
