...
import type { CommandContext, CreateCliOptions } from '../types/cli';
 ...
 */
const DEFAULT_ERROR_HANDLER_CONFIG: ErrorHandlerSecurityOptions = {
  timeout: 5000, // 5 seconds timeout
  allowedModules: ['util', 'path', 'node:util', 'node:path'], // Only safe Node.js modules (both legacy and protocol forms)
  maxFunctionLength: 10000, // Maximum function size in characters
};
...
export function validateErrorHandler(
  ...
  if (config.maxFunctionLength && functionSource.length > config.maxFunctionLength) {
    violations.push(
      `FUNCTION_TOO_LARGE: Function source exceeds ${config.maxFunctionLength} characters`
    );
  }
  ...
  const dangerousPatterns = [
    ...
    // Process manipulation
    {
      ...
      reason: 'process.exit() can terminate the application unexpectedly',
    },
    ...
    ...(config.strict
      ? [
          ...
          },
          {
            ...
            reason: 'Child process spawning is restricted',
          },
          {
            pattern: /require\s*\(\s*['"`]os['"`]\s*\)/gi,
            reason: 'OS module access is restricted',
          },
          {
            pattern: /require\s*\(\s*['"`]crypto['"`]\s*\).*randomBytes/gi,
            reason: 'Large crypto operations are restricted',
          },
        ]
      : []),
    ...
    { pattern: /require\s*\(\s*['"`]http['"`]\s*\)/gi, reason: 'HTTP module access is restricted' },
    {
      ...
      reason: 'HTTPS module access is restricted',
    },
    {
      pattern: /require\s*\(\s*['"`]net['"`]\s*\)/gi,
      reason: 'Network module access is restricted',
    },
  ...
  match = modulePattern.exec(functionSource);
  while (match !== null) {
    const moduleName = match[1];
    if (config.allowedModules && !config.allowedModules.includes(moduleName)) {
...
export async function executeErrorHandlerSafely(
  ...
  const { sanitizeErrorObjectWithMemoryProtection, DEFAULT_MEMORY_CONFIG } = await import(
    './foundation/memory/protection.js'
  );
...
function formatErrorForDisplay(error: Error, options: { showStack?: boolean } = {}): string {
  ...
  const sanitizedMessage = showDetails
    ? error.message
    : enhancedSanitizeErrorMessage(error.message);
...
function truncateErrorMessage(
  message: string,
  maxLength = DEFAULT_SECURITY_CONFIG.maxMessageLength
): string {
...
function sanitizeErrorObject(error: Error, securityConfig = DEFAULT_SECURITY_CONFIG): Error {
  ...
  if (securityConfig.enableMemoryMonitoring) {
    ...
    if (memorySize > securityConfig.maxErrorObjectSize) {
      console.warn(
        `[Security] Large error object detected (${memorySize} bytes), applying memory protection`
      );
    }
  }
  ...
  if (error instanceof CLIError && error.context) {
    const contextEntries = Object.entries(error.context).slice(
      0,
...
export async function createCLI(options: CreateCliOptions): Promise<EnhancedCommand> {
  ...
  if (options.commandsPath) {
    const paths = Array.isArray(options.commandsPath)
  ...
  enhancedProgram.run = async function (argv: string[] = process.argv): Promise<void> {
    ...
    if (!state) {
      throw new Error('CLI state not properly initialized');
    }
    ...
    try {
      ...
      if (!autocompleteConfig.shells || autocompleteConfig.shells.includes(shell)) {
        ...
        if (result.success && result.restartRequired) {
          logger.note(
            `Shell completion installed! Restart your shell or run: ${result.activationCommand}`
          );
        }
      }
    } catch (error) {
      ...
      logger.debug(
        `Autocomplete setup failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
...
export { sanitizeErrorObject, truncateErrorMessage, getObjectMemorySize, formatErrorForDisplay };
