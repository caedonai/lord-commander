...
async function getBundleMetrics(): Promise<{ coreKB: number; fullKB: number; reductionPercent: number }> {
  try {
    ...
    console.log(`📦 Bundle analysis: Core ${coreKB}KB, Full ${fullKB}KB, ${reductionPercent}% reduction`);
...
async function updateReadmeMetrics(metrics: ProjectMetrics): Promise<void> {
  ...
  const updates = [
    ...
    {
      pattern: /- 📦 \*\*[\d.]+KB core bundle\*\* - \d+% smaller with tree-shaking vs \d+KB full SDK/,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log('\n📋 Collected Metrics:');
    console.log(`   Startup: ${metrics.startup.averageMs}ms (vs ${metrics.startup.industry}ms industry)`);
    console.log(`   Bundle: ${metrics.bundleSize.coreKB}KB core, ${metrics.bundleSize.fullKB}KB full (${metrics.bundleSize.reductionPercent}% reduction)`);
    console.log(`   Security: ${metrics.security.totalTests} tests, ${metrics.security.vulnerabilities === 0 ? 'zero' : metrics.security.vulnerabilities} vulnerabilities`);
    console.log(`   API: ${metrics.api.totalExports} exports across ${metrics.api.coreModules} modules`);
...
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1]?.endsWith('update-readme-metrics.ts')) {
  main().catch(error => {
