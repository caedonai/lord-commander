...
describe('Memory Protection Framework Integration (Task 1.5.1)', () => {
  ...
  describe('MemoryViolationAnalyzer - Analysis and Recommendations', () => {
    ...
    it('should calculate security scores based on violations', () => {
      ...
      const multipleAnalysis = analyzer.analyzeMemoryUsage(50000, [
        criticalViolation,
        highViolation,
      ]);
    ...
    it('should generate appropriate recommendations', () => {
      ...
      expect(analysis.recommendations.some((r: string) => r.includes('object complexity'))).toBe(
        true
      );
      expect(analysis.recommendations.some((r: string) => r.includes('object properties'))).toBe(
        true
      );
  ...
  describe('Edge Cases and Attack Scenarios', () => {
    ...
    it('should handle concurrent memory operations safely', async () => {
      ...
      const operations = Array(50)
        .fill(0)
        .map((_, i) => manager.protectOperation(() => `Result ${i}`));
