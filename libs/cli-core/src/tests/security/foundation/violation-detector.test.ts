...
describe('SecurityViolationDetector - Centralized Violation Detection API', () => {
  ...
  describe('Advanced Risk Scoring Algorithm', () => {
    ...
    it('should cap risk scores at 100', async () => {
      const extremePayload =
        // biome-ignore lint/suspicious/noTemplateCurlyInString: Testing security payload
        '../../../etc/passwd; rm -rf /; eval("malicious"); ${Runtime.exec("calc")}';
  ...
  describe('Attack Pattern Correlation', () => {
    it('should detect multi-vector attacks', async () => {
      ...
      if (result.violations.length > 1) {
        ...
        const multiVector = result.attackCorrelations.find(
          (c) => c.attackPattern === 'multi-vector-attack'
        );
        expect(multiVector).toBeDefined();
        expect(multiVector?.sophisticationLevel).toBeOneOf([
          'basic',
          'intermediate',
          'advanced',
          'expert',
        ]);
        expect(multiVector?.recommendedResponse).toBeOneOf([
          'monitor',
          'warn',
          'block',
          'escalate',
    ...
    it('should correlate attacks across sessions', async () => {
      ...
      const sessionCorrelation = result.attackCorrelations.find(
        (c) => c.attackPattern === 'session-based-attack'
      );
  ...
  describe('Recommended Actions Generation', () => {
    ...
    it('should recommend escalation for attack correlations', async () => {
      ...
      if (result.attackCorrelations.length > 0) {
        const escalationActions = result.recommendedActions.filter(
          (a) => a.description.includes('correlat') || a.description.includes('attack patterns')
        );
  ...
  describe('Default Instance and Configuration', () => {
    ...
    it('should have comprehensive default risk scoring config', () => {
      ...
      expect(DEFAULT_RISK_SCORING_CONFIG.attackVectorWeights).toHaveProperty(
  ...
  describe('Violation Metadata and Tracking', () => {
    ...
    it('should track risk factors for violations', async () => {
      ...
      if (result.violations.length > 0) {
        ...
        const envFactor = result.violations[0].riskFactors.find(
          (rf) => rf.type === 'environmental'
        );
