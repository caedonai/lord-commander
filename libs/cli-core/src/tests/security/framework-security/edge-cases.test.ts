...
describe('Framework Security Edge Cases', () => {
  ...
  describe('Malformed JSON Handling', () => {
    it('should handle malformed package.json gracefully', async () => {
      ...
      if (framework) {
        expect(framework.dependencies.security.untrustedSources).toContain(
  ...
  describe('Configuration File Attacks', () => {
    it('should detect path traversal in config file content', async () => {
      await writeFile(
        join(edgeTestDir, 'next.config.js'),
        `
  ...
  describe('Concurrent Access Edge Cases', () => {
    it('should handle concurrent framework detection calls', async () => {
      ...
      const promises = Array.from({ length: 5 }, () => detectFrameworkSecurely(edgeTestDir));
