...
describe('registerCommands Built-in Exclusion', () => {
  ...
  beforeEach(async () => {
    ...
    // Create a temporary directory for test commands within libs/cli-core/src/tests to pass security validation
    const testBaseDir = join(process.cwd(), 'libs', 'cli-core', 'src', 'tests');
    ...
    try {
      ...
      for (const dir of tempDirs) {
        try {
          ...
          console.warn(
            `Failed to cleanup orphaned temp directory ${dir}:`,
            error instanceof Error ? error.message : String(error)
          );
        }
      }
  ...
  describe('Built-in Command Exclusion', () => {
    ...
    it('should register user commands with similar names but different extensions', async () => {
      ...
      await writeFile(
        join(tempDir, 'completion-status.ts'),
        `
        export default function(program, context) {
