...
import { copyFile, ensureDir, exists } from '../core/execution/fs.js';
...
async function getFilesBetweenTags(
  ...
  try {
    ...
    const nameStatusResult = await execa('git', ['diff', '--name-status', `${fromTag}..${toTag}`], {
      cwd,
    });
 ...
 */
async function isBreakingChange(fromTag: string, toTag: string, cwd: string): Promise<boolean> {
  try {
...
export async function createUpdatePlan(
  ...
  try {
    ...
    const strategy: UpdateStrategy = {
      ...
      preserveFiles: options.strategy?.preserveFiles || ['package.json', 'README.md', '.gitignore'],
 ...
 */
async function detectConflicts(diff: VersionDiff, targetDir: string): Promise<UpdateConflict[]> {
