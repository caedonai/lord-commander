  ...
  private calculateSizeInternal(obj: unknown, visited: WeakSet<object>, depth: number): number {
    ...
    if (typeof obj === 'object' && obj !== null) {
      ...
      if (this.config.enableMemoryCache && this.cache.has(obj)) {
        const cachedSize = this.cache.get(obj);
...
export class MemoryViolationAnalyzer {
  ...
  private determineUsageLevel(size: number, violations: MemoryViolation[]): MemoryUsageLevel {
    ...
    if (
      violations.some(
        (v) => v.type === 'object-size-exceeded' || v.type === 'context-size-exceeded'
      )
    ) {
      return 'exceeded';
    }
  ...
  private generateRecommendations(
    ...
    switch (usageLevel) {
      case 'exceeded':
        recommendations.push(
          'CRITICAL: Memory usage exceeds safe limits. Implement immediate reduction measures.'
        );
        break;
      case 'critical':
    ...
    if (violationTypes.has('object-size-exceeded')) {
      recommendations.push(
        'Reduce object complexity by splitting large objects into smaller components.'
      );
    }

    if (violationTypes.has('property-count-exceeded')) {
      recommendations.push(
        'Limit object properties and consider using Map for dynamic key-value storage.'
      );
    }

    if (violationTypes.has('circular-reference-detected')) {
      recommendations.push(
        'Remove circular references to prevent memory leaks and calculation issues.'
      );
    }

    if (violationTypes.has('memory-exhaustion-attempt')) {
      recommendations.push(
        'SECURITY: Potential DoS attack detected. Implement stricter input validation.'
      );
    }
    ...
    if (violations.length > 3) {
      recommendations.push(
        'Multiple violations detected. Consider implementing comprehensive input validation.'
      );
    }
    ...
    try {
      ...
      try {
        ...
        if (resultSize > this.config.maxObjectSize) {
          ...
          warnings.push(
            `Operation result exceeds memory limits (${resultSize} > ${this.config.maxObjectSize} bytes)`
          );
      ...
      if (memoryAnalysis.requiresAction || protectionApplied) {
        ...
        if (
          this.config.enableGarbageCollection &&
          (memoryAnalysis.usageLevel === 'critical' || memoryAnalysis.usageLevel === 'exceeded')
        ) {
...
export function sanitizeErrorObjectWithMemoryProtection(
  ...
  const truncatedMessage =
    originalMessage.length > config.maxMessageLength
      ? originalMessage.slice(0, config.maxMessageLength - 50) +
        'Error message truncated for memory protection'
      : originalMessage;
  ...
  try {
    ...
    for (const key in context) {
      try {
        ...
        warnings.push(
          `Memory protection error: ${error instanceof Error ? error.message : String(error)}`
        );
    ...
    if (
      analysis.usageLevel === 'safe' &&
      analysis.totalSize <= config.maxContextSize &&
      analysis.violations.length === 0
    ) {
    ...
    warnings.push(
      `Context truncated due to memory protection (${analysis.totalSize} > ${config.maxContextSize} bytes)`
    );
