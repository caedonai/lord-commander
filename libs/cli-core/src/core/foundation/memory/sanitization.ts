...
export class AdvancedObjectSanitizer {
  ...
  private sanitizeFunction(
    ...
    // Check for custom strategies first
    if (this.config.customStrategies.has('function')) {
      const strategy = this.config.customStrategies.get('function');
      if (strategy === 'preserve') {
        return func; // Preserve the original function
      }
      if (strategy === 'redact' || strategy === 'remove') {
    ...
    // Default behavior when sanitizeFunctions is false and no custom strategy
    return func;
  }
    ...
    for (const key of limitedKeys) {
      ...
      if (this.config.removePrototypeProperties && this.isDangerousProperty(key)) {
        violations.push({
          ...
          originalValue: (obj as Record<string, unknown>)[key] as SanitizableValue,
      ...
      try {
        ...
        try {
          propertyValue = (obj as Record<string, unknown>)[key];
        } catch (propertyError) {
          warnings.push(
        ...
        const value = await this.deepSanitizeObject(
          propertyValue as SanitizableValue,
          propertyPath,
          violations,
          warnings,
          depth + 1,
  ...
  private redactObject(obj: SanitizableValue, type: ObjectType, _path: string): SanitizableValue {
    switch (type) {
      case 'function':
        return `[Function: ${(obj as unknown as Function)?.name || 'anonymous'}]`;
      case 'circular':
        return '[Circular Reference]';
      case 'buffer':
        return `[Buffer: ${(obj as unknown as Buffer)?.length || 0} bytes]`;
      case 'class-instance':
        return `[${(obj as unknown as Record<string, unknown>)?.constructor?.name || 'Object'} Instance]`;
      case 'unknown':
  ...
  private truncateObject(obj: SanitizableValue, type: ObjectType): SanitizableValue {
    ...
    if (type === 'plain-object' || type === 'class-instance') {
      ...
      for (let i = 0; i < maxKeys && i < keys.length; i++) {
        truncated[keys[i]] = (obj as Record<string, SanitizableValue>)[keys[i]];
      }
  ...
  private hasCircularReferences(obj: unknown): boolean {
    ...
    const check = (current: SanitizableValue): boolean => {
      ...
      try {
        ...
        for (const key of keys) {
          try {
            const value = (current as Record<string, unknown>)[key] as SanitizableValue;
    ...
    return check(obj as SanitizableValue);
  }
  ...
  private calculateObjectDepth(obj: unknown): number {
    ...
    return getDepth(obj as SanitizableValue);
  }
