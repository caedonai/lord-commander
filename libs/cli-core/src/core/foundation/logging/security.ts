 ...
 */
const DEFAULT_LOG_INJECTION_CONFIG: Required<Omit<LogInjectionConfig, 'onSecurityViolation'>> & {
  onSecurityViolation?: (violation: LogSecurityViolation) => void;
} = {
...
const TERMINAL_ATTACK_PATTERNS = {
  ...
  EVAL_ATTEMPTS:
    /eval\s*[([]|exec\s*[([]|system\s*[([]|window\s*\[\s*["']\s*e\s*["']\s*\+\s*["']\s*val\s*["']\s*\]|window\s*\[\s*["'][^"']*e[^"']*val[^"']*["']\s*\]|globalThis\s*\[\s*["'][^"']*eval[^"']*["']\s*\]/gi, // Code execution attempts (enhanced)
...
function applySanitizationRules(
  ...
  return rules.reduce(
    (sanitized, rule) => applySanitizationRule(sanitized, rule, violations, isStrict, isPermissive),
    message
  );
}
 ...
 */
function validateAndSanitizeConfig(config: Partial<LogInjectionConfig>): Required<
  Omit<LogInjectionConfig, 'onSecurityViolation'>
> & {
  onSecurityViolation?: (violation: LogSecurityViolation) => void;
} {
  ...
  if (!['strict', 'standard', 'permissive'].includes(validated.protectionLevel)) {
    validated.logger.warn(
      `[Log Security] Invalid protection level: ${validated.protectionLevel}. Using 'standard'.`
    );
  ...
  if (typeof validated.maxLineLength !== 'number' || validated.maxLineLength < 0) {
    validated.logger.warn(
      `[Log Security] Invalid maxLineLength: ${validated.maxLineLength}. Using default 2000.`
    );
  ...
  if (typeof validated.warningThreshold !== 'number' || validated.warningThreshold < 0) {
    validated.logger.warn(
      `[Log Security] Invalid warningThreshold: ${validated.warningThreshold}. Using default 1000.`
    );
  ...
  if (
    ...
    typeof validated.onSecurityViolation !== 'function'
  ) {
...
export function sanitizeLogOutputAdvanced(
  ...
  if (finalConfig.detectTerminalManipulation) {
    ...
    sanitized = applySanitizationRules(
      sanitized,
      terminalRules,
      violations,
      isStrict,
  ...
  if (!finalConfig.preserveFormatting) {
    if (TERMINAL_ATTACK_PATTERNS.LOG_INJECTION.test(sanitized)) {
      violations.push(
        createViolation('line-injection', 'high', 'Line ending injection detected', sanitized)
      );
      sanitized = sanitized
        .replace(/\r\n/g, ' [CRLF] ')
        .replace(/\r/g, ' [CR] ')
  ...
  if (finalConfig.detectHyperlinkInjection) {
    if (TERMINAL_ATTACK_PATTERNS.HYPERLINKS.test(sanitized)) {
      violations.push(
        createViolation('hyperlink-injection', 'medium', 'Terminal hyperlinks detected', sanitized)
      );
    ...
    if (isStrict) {
      if (TERMINAL_ATTACK_PATTERNS.URL_PATTERNS.test(sanitized)) {
        violations.push(
          createViolation(
            'hyperlink-injection',
            'low',
            'HTTP URLs detected in strict mode',
            sanitized
          )
        );
      ...
      if (TERMINAL_ATTACK_PATTERNS.FILE_URLS.test(sanitized)) {
        violations.push(
          createViolation('hyperlink-injection', 'medium', 'File URLs detected', sanitized)
        );
  ...
  if (TERMINAL_ATTACK_PATTERNS.NULL_BYTES.test(sanitized)) {
    violations.push(
      createViolation('line-injection', 'high', 'Null byte injection detected', sanitized)
    );
  ...
  for (const pattern of finalConfig.customDangerousPatterns || []) {
    if (pattern.test(sanitized)) {
      violations.push(
        createViolation('custom-pattern', 'medium', 'Custom dangerous pattern detected', sanitized)
      );
  ...
  if (sanitized.length > finalConfig.maxLineLength) {
    violations.push(
      createViolation('length-overflow', 'medium', 'Message length exceeds maximum', sanitized)
    );
  ...
  if (violations.length > 0) {
    ...
    if (originalLength > finalConfig.warningThreshold) {
      finalConfig.logger.warn(
        `[Log Security] Large message detected: ${originalLength} chars, potential DoS attempt`
      );
    }

    if (violations.some((v) => v.severity === 'critical')) {
      finalConfig.logger.warn(
        `[Log Security] CRITICAL: ${violations.filter((v) => v.severity === 'critical').length} critical violations detected`
      );
    }
    ...
    if (finalConfig.onSecurityViolation) {
      violations.forEach((violation) => {
        ...
        try {
          ...
          finalConfig.logger.warn(
            `[Log Security] Violation callback error: ${callbackError instanceof Error ? callbackError.message : String(callbackError)}`
          );
...
export function analyzeLogSecurity(message: string): LogSecurityAnalysis {
  ...
  const hasAnsiEscapes =
    TERMINAL_ATTACK_PATTERNS.ANSI_CSI.test(message) ||
  ...
  const hasTerminalManipulation =
    TERMINAL_ATTACK_PATTERNS.TERMINAL_TITLE.test(message) ||
  ...
  if (hasTerminalManipulation) {
    ...
    if (TERMINAL_ATTACK_PATTERNS.TERMINAL_RESET.test(message)) {
      violations.push(
        createViolation('terminal-manipulation', 'critical', 'Terminal reset detected', message)
      );
      riskScore += 20;
    }
  }
  ...
  if (hasLineInjection) {
    ...
    violations.push(
      createViolation('line-injection', 'high', 'Line ending injection detected', message)
    );
  }
  ...
  const hasUnicodeAttacks =
    TERMINAL_ATTACK_PATTERNS.BIDI_OVERRIDE.test(message) ||
  ...
  const hasCommandExecution =
    TERMINAL_ATTACK_PATTERNS.SHELL_COMMANDS.test(message) ||
  ...
  const hasHyperlinkInjection =
    TERMINAL_ATTACK_PATTERNS.HYPERLINKS.test(message) ||
  ...
  if (hasNullByteInjection) {
    ...
    violations.push(
      createViolation('line-injection', 'high', 'Null byte injection detected', message)
    );
  }
  ...
  if (hasExcessiveLength) {
    ...
    warnings.push(
      `Extremely long message detected (${message.length} chars) - potential DoS attack`
    );
    ...
    if (message.length > 50000) {
      violations.push(
        createViolation('length-overflow', 'critical', 'Massive message length detected', message)
      );
    }
  }
...
export class LogSecurityMonitor {
  ...
  private handleViolations(violations: LogSecurityViolation[], source?: string) {
    ...
    const violationCount = this.violationCounts.get(sourceKey) || 0;
    if (violationCount >= this.alertThreshold) {
      ...
      if (this.onAlert) {
        ...
        this.logger.error(
          `[SECURITY ALERT] ${alert.severity.toUpperCase()}: ${alert.violationCount} violations from ${sourceKey}`
        );
      }
